...
- One pre-existing, low-severity advisory finding, not introduced or worsened by this release: `internal.prevent_variation_number_counter_decrease()` does not set `search_path = ''`. Not exploitable — the function references nothing unqualified — but inconsistent with this migration's own stated convention of setting it everywhere. **Follow-up confirmed:** fix it as the first commit after Sprint 3 merges (or a tiny standalone maintenance migration/PR before Sprint 4 if that's cleaner) — not applied in this release, not urgent, but tracked so it doesn't get lost. Keeping `search_path` conventions consistent across every security-relevant function is worth the small follow-up even though the practical risk here is low.
