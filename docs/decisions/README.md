...
## ADR-016: Dedicated, Strongly Typed Table Per Document Type — Not a Generic `documents` Table

**Date:** 2026-07-31
**Status:** Accepted

**Context:** Planning the migration of BIK's ~20 document-generating tools (Variation Notices, Progress Claims, Quotes, Defect Reports, Site Diaries, and so on) from `localStorage` onto the Supabase backend, starting with a Variation Notices pilot. `organisations`/`profiles`/`customers`/`projects` (001–004) each got their own dedicated table.

An earlier draft of this decision proposed the opposite: a single generic `public.documents` table with a `type` discriminator and a `fields` jsonb payload, on the reasoning that ~20 near-identical migrations and RLS policy blocks was worse than one shared table. That draft was built and drafted as `010_create_documents.sql` before this decision was made explicitly and corrected the approach: the generic shape traded away exactly the thing this codebase has insisted on everywhere else — real, DB-level typed/constrained columns for anything with business meaning — in exchange for maintenance convenience that a working field inventory shows isn't actually needed. Inspecting `js/tools/variation-notice/config.js`'s full field list (`docs/PHASE_3_VARIATION_NOTICES_SCHEMA.md`) found that essentially every field either affects calculation/status/reporting/filtering (and therefore belongs as a typed column regardless of which table shape is chosen) or is fixed, always-present content of this specific document type (not "genuinely variable secondary data" just because it's long-form text). Almost nothing in a real document actually needs jsonb once looked at field-by-field — one column (`issued_snapshot`, a derived point-in-time copy) turned out to be the only genuine fit.

Two tools remain explicitly excluded from this decision's scope: Attendance and Site Diary are not form-engine documents (Attendance has its own QR check-in/check-out model; Site Diary has a bespoke recent redesign) and will get their own dedicated schema when their turn comes.

**Decision:** Each document type gets its own dedicated table, following the exact `organisations`/`profiles`/`customers`/`projects` precedent rather than departing from it. The pilot is `010_create_variation_notices.sql` (`public.variation_notices`) — draft, not yet applied; see `docs/PHASE_3_VARIATION_NOTICES_SCHEMA.md` for the full field-by-field inventory and `supabase/migrations/010_create_variation_notices.sql` for the schema itself. `jsonb` is used for exactly one column, `issued_snapshot` — a frozen copy of the typed columns at the moment of issue, genuinely derived/secondary data, not the document's primary content. Every other field with any bearing on workflow, filtering, reporting, calculation, or integration is a real typed column: money as integer cents (`cost_excl_gst_cents`, matching `projects.estimated_contract_value_cents`'s existing convention), enums as `text` + `CHECK` (matching `projects.status`/`profiles.role`), GST/total as `GENERATED ALWAYS AS ... STORED` columns computed directly from their inputs so they can never independently drift. `project_id` stays `not null` (unlike `projects.customer_id`'s optional relationship) and the same cross-tenant `project_id`-belongs-to-`organisation_id` integrity trigger from the earlier draft is preserved unchanged — that concern doesn't depend on which table shape was chosen. RLS reuses `internal.current_organisation_id()` (005) directly, same shape as `customers`/`projects`. A client-details snapshot (`client_name`, `client_email`) is stored directly on the row — a plain denormalised copy, decoupled from any live `customers` record, rather than a jsonb blob; "snapshot" describes decoupling from the live record, not a particular column type.

**Rationale:**
- This codebase's standing practice, everywhere else, is: constrain at the database layer whatever has business meaning, and treat "the frontend validates it" as insufficient on its own. A shared `fields` jsonb payload for an entire document's actual content — the reason the document exists at all — is a much larger exception to that practice than any other trade-off this schema has made, and the field inventory shows it wasn't buying much: the fields that would have justified it (large, freeform, unpredictable) mostly don't exist in a real Variation Notice. What exists is a fixed set of well-known fields, some long-form, none actually variable in shape.
- Per-type tables keep money, dates, and enums exactly as constrainable, indexable, and queryable as `organisations`/`profiles`/`customers`/`projects` already are — no regression in what this schema can guarantee about a row's own content, only about which table it's the twentieth vs the fifth new one.
- The migration-count cost the earlier draft was optimising away is real but bounded and precedented: `001`–`004` already established that one new table per real, distinct product entity is this codebase's normal cost of doing business, reviewed and applied one at a time exactly like every other migration here.
- Learning C2/ADR-014's lesson directly, unchanged from the earlier draft: this migration issues its own explicit `GRANT SELECT, INSERT, UPDATE ... TO authenticated` rather than assuming any privilege is inherited from `008`/`009`'s corrected default ACL, which only ever covered the *dangerous* extra privileges, never made the useful ones default for future tables.

**Consequences (+):** No loss of database-level guarantees relative to the rest of this schema — money, status, dates, and enums on every document type stay real, typed, constrained columns, filterable and indexable natively. The cross-tenant project/organisation integrity gap is closed explicitly, unchanged from the earlier draft. Each tool's migration is small, self-contained, and independently reviewable, matching every other migration in this repo.
**Consequences (−):** No single "all documents across every type" query without a `UNION ALL` once more tables exist — a real capability given up, traded for stronger per-type guarantees. ~19 more tool migrations ahead, each its own reviewed PR, rather than one shared schema absorbing all of them; accepted as the same normal cost `001`–`004` already set the precedent for, not a new kind of overhead.

**Alternatives considered:** The generic `documents` table drafted first (rejected — see Context; the maintenance-cost argument for it didn't hold up against an actual field inventory, and it traded away this codebase's standing database-level-validation practice for a benefit that turned out to be smaller than assumed); a hybrid (generic table for "simple" tools, dedicated tables only for complex ones) (rejected — "simple" turned out, on inspection, to mean "still has typed fields for calculation/reporting," which is every tool; a threshold that collapses to "always dedicated" on inspection isn't a real middle option); requiring `customer_id` directly on each document table in addition to `project_id` (rejected for now, same reasoning as the earlier draft — every document already reaches its customer transitively via `project.customer_id`; add a direct link later if a concrete need for it is found, per `004`'s own review note on this exact question for projects).
