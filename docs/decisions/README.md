...
## ADR-016: One Generic `documents` Table Shared Across Document-Generating Tools, Not a Table Per Type

**Date:** 2026-07-31
**Status:** Proposed

**Context:** Planning the migration of BIK's ~20 document-generating tools (Variation Notices, Progress Claims, Quotes, Defect Reports, Site Diaries, and so on) from `localStorage` onto the Supabase backend, starting with a Variation Notices pilot. `organisations`/`profiles`/`customers`/`projects` (001–004) each got their own dedicated table, and the first draft of this decision assumed the same pattern would continue: one table per document type. Seeing the full tool list changed that assessment — most of these tools are structurally identical (the same `js/toolkit/engine.js`/`renderer.js`/`exporter.js` plumbing, differing only by each tool's own `config.js` field schema), so a table-per-type approach means roughly twenty near-duplicate migrations and RLS policy sets for what is, at the schema level, the same shape every time: fields + status + a project + an organisation.

Two tools are explicitly excluded from this decision's scope: Attendance and Site Diary are not form-engine documents (Attendance has its own QR check-in/check-out model; Site Diary has a bespoke recent redesign) and will get their own dedicated schema when their turn comes, not be forced into this table.

**Decision:** Add `010_create_documents.sql` (draft — see `docs/PHASE_3_DOCUMENTS_SCHEMA.md` for the full design and `supabase/migrations/010_create_documents.sql` for the schema itself), a single `public.documents` table with a `type` text column (CHECK-constrained, extended by one `ALTER TABLE` per future tool migration) as the discriminator, and a `fields` jsonb column holding each document's actual form data. Scoped to exactly one `organisation_id` and one `project_id` (not null, unlike `projects.customer_id`'s optional relationship). RLS reuses `internal.current_organisation_id()` (005) directly — no new helper function needed, the exact same read/create/update-within-own-org shape as `customers`/`projects`. A dedicated trigger (`enforce_document_project_same_organisation`) closes a specific integrity gap this shape introduces: without it, nothing stops `project_id` from referencing a project belonging to a *different* organisation than `organisation_id` claims, since a foreign key alone only confirms the project exists somewhere, not that it's the caller's own.

**Rationale:**
- At four tables, per-type schemas cost little and buy strong typing. At twenty, the cost inverts: twenty migrations and twenty copy-pasted RLS policy blocks to maintain, versus one migration and one policy set that already covers every future tool.
- A shared table gets a cross-tool "all documents for this project" or "all documents in this organisation" view for free — a plain `select ... from documents where project_id = ...`, no `UNION` across per-type tables required. That is a real product capability (a project's full document history in one query), not just an implementation convenience.
- The cost is real and named rather than hidden: `fields` is not schema-validated by Postgres, so a malformed or type-mismatched field value is only caught by each tool's own `config.js`, not by a database constraint. Acceptable here because these are draft/internal-editing fields, not the kind of value (money, dates driving business logic, tenancy) this codebase has otherwise insisted on constraining at the database layer — those stay as real columns (`status`, `document_number`, `type` itself, the audit fields) on the shared table.
- `project_id not null` (versus `projects.customer_id`'s nullable, optional relationship) reflects a genuine difference in the two relationships: a project may legitimately predate a formal customer record (003's review), but a document produced by a project-scoped tool without a project doesn't fit any current product flow. If a "standalone document, no project" capability is ever wanted, that is a deliberate future decision, not a default this table falls into.
- Learning C2/ADR-014's lesson directly: this migration issues its own explicit `GRANT SELECT, INSERT, UPDATE ... TO authenticated` rather than assuming any privilege is inherited. ADR-014/ADR-015 corrected the *default* ACL only for the dangerous `MAINTAIN`/`REFERENCES`/`TRIGGER`/`TRUNCATE` set; they deliberately left `SELECT`/`INSERT`/`UPDATE` non-default for future tables, so a new table starts at zero client-role privileges regardless of how carefully `008`/`009` were written. Getting this right in the same migration that creates the table, rather than rediscovering it during live deployment validation a second time, is the entire point of writing it down here.

**Consequences (+):** One migration and one RLS policy set now covers Variation Notices and every subsequent form-engine tool migration — the four remaining planned batches (docs/decisions/README.md's tool-migration plan, communicated in-conversation) each add at most one `ALTER TABLE ... type` extension, not a new table. A project's full document history across every tool type is a single, simple query. The cross-tenant project/organisation integrity gap this shape introduces is closed explicitly, not left implicit.
**Consequences (−):** No per-field database constraint on any document type's actual content — validation lives entirely in application code (`config.js` per tool), a weaker guarantee than the rest of this schema provides for its typed columns. A single wide table is also a long-term risk if document types' needs diverge enough that the shared shape stops fitting well (e.g. a type that genuinely needs its own strongly-typed relational sub-structure, not just a jsonb blob) — if that happens for a specific type, breaking just that type out into its own dedicated table remains available as a later, narrowly-scoped decision, not precluded by this one.

**Alternatives considered:** A dedicated table per document type, matching `organisations`/`profiles`/`customers`/`projects`'s own pattern (rejected at this scale — see Rationale; remains the better choice for a type whose needs genuinely diverge, and nothing here prevents adopting it for one type later); a fully generic `entity`/`records` table spanning documents *and* future non-document data like Attendance (rejected — Attendance and Site Diary are already known to need a different shape, and generalising further than "documents" invites exactly the kind of schema-integrity erosion `fields` already trades away once; keeping the generic table scoped to "things `js/toolkit/engine.js` produces" is a narrower, more defensible boundary); requiring `customer_id` directly on `documents` in addition to `project_id` (rejected for now — every document already reaches its customer transitively via `project.customer_id`; a direct link can be added later if a concrete need for it is found, per `004`'s own review note on this exact question for projects).
