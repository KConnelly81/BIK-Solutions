...
| **C3 — critical defect found (post-Stage-4, during a repeat of Stage 1's grant check)** | **Blocked — validation paused** | 2026-07-29 | Claude (BIK Phase 2) | `anon` and `authenticated` both hold `MAINTAIN`/`REFERENCES`/`TRIGGER`/`TRUNCATE` on all four tables, inherited from a pre-existing `postgres`-role default ACL on `public` (project provisioning, not any BIK migration). `TRUNCATE` is not filtered by RLS at all — confirmed live and rolled back: `set local role anon; truncate public.projects;` succeeded. This is a critical, RLS-bypassing, unauthenticated data-destruction path. See C3 in `docs/PHASE_1_DATABASE_REVIEW.md` and ADR-015 for the full investigation, privilege matrix, and analysis. Corrective migration `009_revoke_dangerous_table_privileges.sql` is drafted and documented, **not yet applied**, pending explicit approval — per standing instruction. **Stage 5 onward, and all frontend/application work, are paused until `009` is approved, applied, and re-verified.** |
| Stage 5 — Role and suspension behaviour | Blocked | | | Paused pending C3/`009` — see above. Not a consequence of any Stage 3/4 failure; those passed cleanly. |
| Stage 6 — Archive and deletion behaviour | Blocked | | | Same reason as Stage 5. |
| Stage 7 — Last-owner invariant | Blocked | | | Same reason as Stage 5. |
| Test data cleanup | Not started | | | Two disposable `auth.users` rows now have live test data: `disposable-a` owns Org A (`cb8fc16e-dca0-44ab-81ef-6e48391ed260`), `disposable-b` owns Org B (`72cbb5e5-9bdc-4ca8-b854-49f25331713c`). `disposable-c` remains unused, reserved for Stage 5's member-role fixture. Cleanup deferred until Stage 5–7 complete, per the runbook's own ordering. |
...
- Repository commit hash tested: `60d7a4b` (migrations `001`–`007`); `008` applied and verified in this session; `009` drafted, pending approval
- Error details (if any failure occurred): Stage 2 anonymous-access test originally failed with `permission denied for table organisations` instead of the documented zero-rows expectation — see C2, corrected via `008`, now the documented permanent expectation. A second, unrelated, more severe defect (C3 — `anon`/`authenticated` holding RLS-bypassing `TRUNCATE`, among other unneeded privileges) was found during a repeat of Stage 1's grant check after `008`, and confirmed live (rolled back safely) — see C3 above and ADR-015. `001`–`008` themselves had zero failures across all per-migration verifications, Stage 1, Stage 2 (post-correction), Stage 3, and Stage 4.
- Remediation commit (if a confirmed defect required a migration fix): `008_grant_authenticated_table_privileges.sql` — applied and verified. `009_revoke_dangerous_table_privileges.sql` — drafted, documented (ADR-015), committed to the repository; **not yet applied to `hpcqncghvdrlvufxfdnd`**, pending explicit approval per standing instruction.

**Note:** Migrations `001`–`008` are applied and verified against `hpcqncghvdrlvufxfdnd`, including Stage 1 (static inspection), Stage 2 (anonymous access, corrected expectation), Stage 3 (authenticated bootstrap), and Stage 4 (tenant isolation) — all passed. Validation is **paused** before Stage 5 pending review and application of `009` (C3/ADR-015 — critical privilege defect, unrelated to Stages 1–4's own results). Stage 5 onward, test data cleanup, and all frontend integration work remain blocked until `009` is applied and re-verified.
