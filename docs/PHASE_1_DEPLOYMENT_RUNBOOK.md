...
| Stage 5 — Role and suspension behaviour | Not started | | | Unblocked now that `009` is applied and verified; not yet executed. Not a consequence of any Stage 3/4 failure; those passed cleanly. |
| Stage 6 — Archive and deletion behaviour | Not started | | | Same as Stage 5 — unblocked, not yet executed. |
| Stage 7 — Last-owner invariant | Not started | | | Same as Stage 5 — unblocked, not yet executed. |
...
- Error details (if any failure occurred): Stage 2 anonymous-access test originally failed with `permission denied for table organisations` instead of the documented zero-rows expectation — see C2, corrected via `008`, now the documented permanent expectation. A second, unrelated, more severe defect (C3 — `anon`/`authenticated` holding RLS-bypassing `TRUNCATE`, among other unneeded privileges) was found during a repeat of Stage 1's grant check after `008`, confirmed live (rolled back safely), and corrected via `009` — see C3 above and ADR-015. `001`–`009` themselves had zero failures across all per-migration verifications, Stage 1, Stage 2 (post-correction), Stage 3, and Stage 4.
...
**Note:** Migrations `001`–`009` are all applied and verified against `hpcqncghvdrlvufxfdnd`, including Stage 1 (static inspection, both passes), Stage 2 (anonymous access, corrected expectation), Stage 3 (authenticated bootstrap), and Stage 4 (tenant isolation) — all passed, and C3 is now corrected. Stage 5 onward and test data cleanup have **not yet been executed** and remain the next step before any frontend integration begins.
