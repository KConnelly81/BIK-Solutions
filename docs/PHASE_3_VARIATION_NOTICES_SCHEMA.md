...
4. ~~Wire Variation Generator to it (Sprint 3).~~ Implemented on `sprint-3-variation-notice-frontend`: `variation-generator.html` is gated with the same `requireSession()`/no-flash pattern as `app-dashboard.html`, launched from a new "New Variation Notice" link on each project card there (not from `dashboard.html`). One deviation from the original plan, worth recording: rather than reusing `project-store.js`'s `PROJECT_STORAGE_POINT` (a localStorage-only concept, not actually Supabase-shaped), a new dedicated module (`js/tools/variation-notice/supabase-integration.js`) owns the Supabase project context, and the legacy `ProjectUI` project bar is disabled for this tool specifically (`ToolController`'s new opt-in `disableLegacyProjectUI` flag) — the two project concepts read the same `?project=` URL param for unrelated things and would otherwise collide. Dropping the now-redundant `builderName`/`builderABN`/etc. fields from the form (sourcing them from the organisation join instead) was **not** done this round — kept out of scope as a separate cleanup, not required for the numbering integration itself. **Not yet merged — pending review and the manual test run below.**
5. Manual test checklist: `docs/PHASE_3_SPRINT_3_MANUAL_TEST_STEPS.md` — not yet run against the live deployment (code-level review and unit tests only so far: `js/tools/variation-notice/__tests__/variation-save-logic.test.js`, 32/32 passing). **Must be run and passed before merging Sprint 3 to `main`.**
