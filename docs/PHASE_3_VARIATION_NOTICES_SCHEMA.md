...
### Full local test suite, this round
...
| 3 | Diff-format manual override, the literal user scenario: auto-assign to `"003"`, manually save `"VAR-010"`, continue auto-assigning through `"010"` | Pass — no collision at any point; `"VAR-010"` and `"010"` coexist as distinct rows |
...
**Not built:** reassignment on `UPDATE` (the trigger is `INSERT`-only; clearing an existing row's number back to blank does not trigger a fresh auto-assignment); reclaiming/compacting numbers from abandoned drafts (gaps are expected and accepted — the guarantee is no collision, ever, not no gaps, same as invoice numbering in any accounting system); resetting a project's counter (no product requirement yet).
