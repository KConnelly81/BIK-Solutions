...
- **Sprint 5a (v4) — issue-transition redesign + independent migration reviews**, per explicit direction that a document's status transition should be a controlled RPC, not an ordinary table write. Full detail: `docs/PHASE_5A_DESIGN_PROPOSAL.md` §5 and §9, `docs/PHASE_5A_QUOTES_MIGRATION_REVIEW.md`, `docs/PHASE_5A_PROGRESS_CLAIMS_MIGRATION_REVIEW.md`.
  - **`issue_quote()`/`issue_progress_claim()` are now the only path to `draft → issued`** — authenticated's `UPDATE` grant on `quotes`/`progress_claims` is column-scoped and excludes the entire lifecycle surface (status, issued timestamps, snapshot) and every server-computed total; a plain client `UPDATE` touching any of them now fails with a Postgres permission error, not a business-rule error. Both RPCs are `SECURITY DEFINER`, independently re-checking organisation ownership since they no longer run under RLS (same pattern `011`'s `assign_variation_notice_number()` established).
  - **`issued_by`/`issued_snapshot` columns added to both tables** — a frozen, self-contained copy of the record and its line items exactly as issued, captured entirely inside the existing state-machine trigger.
  - **Progress Claims: a temporary, unconditional issuing gate**, implemented (not merely recommended) — `enforce_progress_claim_status_transition()` refuses every `draft → issued` attempt until the GST/retention calculation method is confirmed. Drafts remain fully usable. Lifting the gate later is a one-line follow-up migration.
  - **One real bug found and fixed by the second dry-run round**: `recalculate_quote_totals()`/`recalculate_progress_claim_totals()` issue their own separate `UPDATE` against the now-restricted header columns, which failed under the invoking role's own (now narrower) privileges. Fixed by making both `SECURITY DEFINER` — safe without an extra ownership check, since the line-item write they fire from was already RLS-permitted to the caller.
  - **Two independent migration review documents** (`docs/PHASE_5A_QUOTES_MIGRATION_REVIEW.md`, `docs/PHASE_5A_PROGRESS_CLAIMS_MIGRATION_REVIEW.md`) — full field/trigger/RPC/grant/index inventory for each, confirming independent deployability from one another, and flagging real gaps found in review: neither table's status enum includes `'archived'` (no soft-delete path for an abandoned draft), Quotes' issue requirements don't cover `quote_type`/`valid_until`, and Progress Claims has no constraint preventing overclaiming (deliberate, but resting on an unconfirmed assumption) and no cross-claim consistency check on contract value.
  - **A migration-structure recommendation, not yet actioned**: splitting each of `012`/`013` into three ordered migrations (core table/RLS/calculations, numbering, issue workflow), mirroring the `010`/`011` precedent, for safer independent rollback of the newest/riskiest layer. Recorded in the design doc §12; requires separate sign-off before any restructuring work begins.
  - Re-verified end-to-end in a second local Postgres 16 dry run, specifically re-testing the privilege boundary (permission-denied on direct status writes, correct RPC behaviour, cross-organisation isolation for the now-`DEFINER` functions) — all 13 migrations (`001`-`013`) still apply cleanly in sequence.
  - **Still not applied to `hpcqncghvdrlvufxfdnd`. No frontend build started. Project Hub still not merged**, pending the manual browser checklist.
- **Sprint 5a (v5) — migration restructure, lifecycle expansion, interim safety constraints**, per explicit direction. Full detail: `docs/PHASE_5A_DESIGN_PROPOSAL.md` §12/§13, `docs/PHASE_5A_QUOTES_MIGRATION_REVIEW.md`, `docs/PHASE_5A_PROGRESS_CLAIMS_MIGRATION_REVIEW.md`, `docs/PHASE_5A_PROJECT_HUB_PR_NOTES.md`.
  - **Migrations restructured from 2 files into 6 layered, ordered, independently appliable files**: `012_create_quotes.sql` (core) / `013_create_quote_numbering.sql` / `014_create_quote_issue_workflow.sql`; `015_create_progress_claims.sql` (core) / `016_create_progress_claim_numbering.sql` / `017_create_progress_claim_issue_workflow.sql` (**BLOCKED**) — mirroring the `010`/`011` precedent already established in this repo. `previously_claimed_cents` derivation moved out of the numbering layer into a new, correctly non-`DEFINER` trigger in the Progress Claims core layer (least-privilege correction — it never needed elevation).
  - **Quotes lifecycle**: `void`/`archived` added to the status enum (7 values). `issue_quote()` now additionally requires `quote_type`, a `valid_until` date `>= quote_date`, and `client_email` (not just `client_name`) — all four new checks individually verified in the local dry run, confirming the exact check order.
  - **Progress Claims lifecycle**: `archived` added to the status enum. New interim constraint, `remaining_value_cents >= 0`, enforced at the core layer for every draft (not deferred to issuing) — prevents overclaiming and "exceeds recognised contract value" simultaneously, since they're the same constraint. `issue_progress_claim()` remains guaranteed-rejecting via its own dedicated, clearly-`BLOCKED` migration.
  - **Every `SECURITY DEFINER` function across all 6 files audited** against a fixed checklist (fixed `search_path`, qualified references, grants revoked from `PUBLIC`/`anon`, granted only where required, independent organisation/project ownership checks rather than reliance on the triggering RLS check, no swallowed unrelated errors) — see the design doc §13 for the full table.
  - **Re-verified end-to-end**: all 17 migrations (`001`-`017`) apply cleanly in sequence; a dedicated functional re-test confirmed every layer boundary (manual numbering still works with only the core layer applied; auto-numbering layers on top cleanly; the tightened issue requirements fire in the correct order; the overclaim rejection; the permission boundary; cross-organisation isolation for every restructured function).
  - **Project Hub separated into its own PR preparation** (`docs/PHASE_5A_PROJECT_HUB_PR_NOTES.md`) — exact 4-file list confirmed, `project.html` and existing tools confirmed untouched. **One blocking dependency finding surfaced, not silently worked around**: `project-hub.html` depends on two Sprint 4 toolkit modules (`js/toolkit/supabase-project-context.js`, `supabase-record-panel.js`) that exist only on the still-open, still-unmerged `sprint-4-shared-integration-pattern` branch (PR #6) — `main` does not have them. PR not yet opened pending a sequencing decision.
  - **Still not applied to `hpcqncghvdrlvufxfdnd`. No frontend build started for either tool. Project Hub PR not yet opened.**
...
- **Production retest after PR #7's merge found the identical "Loading…" symptom still live**, despite the try/catch fix being deployed. Investigated (`hotfix/sprint4-variation-list-loading`, branched from `main` at `2c2f6b8`) rather than assuming the original diagnosis was complete:
  ...
  - 51/51 tests passing (46 existing + 5 new `with-timeout` tests). **PR #8 merged to `main` (`07ee86c`), 2026-08-02.**
- **Live retest after PR #8 still found the identical "Loading…" symptom — with the mount try/catch confirmed deployed and no `[BIK] Variation Notice failed to mount` error, and no other uncaught exception, ever appearing.** This rules out the PR #8 mount-exception theory as the live cause: `refreshVariationsList()` sits unconditionally right after that try/catch, so its absence of a caught error means execution reached past it. Rather than a further guess, added numbered checkpoint logging (`hotfix/sprint4-variation-list-diagnostics`, branched from `main` at `07ee86c`) across the full path — module load, `onGated()` entry, `mount()` start/end, `refreshRecordList()` entry, actual resolved DOM elements (id/class/`hidden`/computed `display`, logging "NOT FOUND" rather than assuming a match), query start/settle, and final rendered outcome — so one more live reload pinpoints the exact stopping statement instead of another round of hypotheses. Also restructured `refreshRecordList()`'s loading-state cleanup into an explicit `try`/`catch`/`finally` (previously an equivalent-but-implicit unconditional line after the `try`/`catch`) and made every DOM write defensive (`if (el) ...` instead of assuming `sb-list-loading`/`sb-list-empty`/`sb-list`/`sb-list-total` are non-null), so a wrong/missing id now degrades safely instead of throwing. Checked the CSS directly before touching anything: `.text-muted` (the class on `#sb-list-loading`) sets only `color`/`font-size`, no `display` — and no global `[hidden]` reset rule covers it — so the speculative `[hidden] { display: none !important; }` safety rule was **not** added. 51/51 tests still passing (diagnostic logging needs no new tests — the underlying outcome logic is unchanged). **PR #9 merged to `main` (`0f55d33`), 2026-08-02.**
- **Live production retest passed.** Confirmed against `main` @ `0f55d33`: Variation Notice loads correctly, project context resolves, the saved-variations list renders (populated and empty states both confirmed, plus a fresh save-and-refresh and repeated reloads), the permanent `Loading…` defect is resolved, and production is serving the corrected code (no `[BIK]` mount-failure error, no uncaught exception). Checklist: `docs/PHASE_4_SPRINT_4_MANUAL_TEST_STEPS.md`.
- **`[BIK-DIAG]` checkpoint logging removed** (`hotfix/sprint4-close-out`) now that the live cause was confirmed fixed — it was diagnostic-only, added for PR #9 specifically to trace the stopping point live. The correctness fixes it was added alongside (the `try`/`catch`/`finally` restructure in `refreshRecordList()`, the defensive `if (el) ...` DOM checks, the `with-timeout.js` guard) all remain; only the `console.log` checkpoints and the `diag()`/`describeEl()` helpers were stripped. 51/51 tests still passing — removing log-only statements changes no observable behaviour the suite covers.
- **Sprint 4 closed.** Tagged `v0.4.0-sprint4` — see `docs/RELEASE_v0.4.0_SPRINT4.md` for the full release record (features, security review, verification summary, known limitations, items deferred to Sprint 5a).
- **Project Hub (frontend) PR opened** (`feature/project-hub` → `main`, draft, not merged) — the single navigation point for a project's tools, built entirely on the now-live Sprint 4 shared modules (`js/toolkit/supabase-project-context.js`/`supabase-record-panel.js`), no new gating or save-panel code. Shows the project header (name, site address, status pill — `project.status` now included in the shared gate's query, needed here for the first time), a tool launch row ("New Variation Notice" live; "New Quote"/"New Progress Claim" shown as disabled "Coming soon" pending Sprint 5a's backend), and the project's variation notices list, reusing the exact rendering already proven in `variation-notice/supabase-integration.js`. `app-dashboard.html`'s project-card action replaced with a single "Open project" link to the hub. Exactly 5 files in this PR: `project-hub.html`, `css/project-hub.css`, the `app-dashboard.html` one-line link change, `docs/PHASE_5A_PROJECT_HUB_MANUAL_TEST_STEPS.md`, `docs/PHASE_5A_PROJECT_HUB_PR_NOTES.md` — no migrations, no Quotes/Progress Claims work. Legacy `project.html` and every other tool confirmed untouched. 51/51 automated tests passing (unaffected — this PR adds no new JS logic of its own). **Draft PR, not merged — awaiting the live manual checklist.**
